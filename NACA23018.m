...
%         end
    ...
    for i=1:n-1
       length(i)=sqrt((x_u(i+1)-x_u(i))^2+ (y_u(i+1)-y_u(i))^2);
       length(2*n-1-i)= sqrt((x_l(i+1)-x_l(i))^2+ (y_l(i+1)-y_l(i))^2);
    end
    V=[v*cos(a),v*sin(a)];
    [lamda,B]=solver(V,control_x,control_y,discrete_x,discrete_y,n,length,Angle);
     
    % 计算切向速度
     
   vel1=B*lamda+(V*[cos(Angle);sin(Angle)])';
